class gocd::install_go_agent_2 {
	...
	#service { 'go-agent-2':
		...
		#subscribe => [File[
			...
			#"/usr/share/go-agent-2",
			#"/etc/default/go-agent-2",
			#"/var/log/go-agent-2",
			#"/opt/agents/go-agent-2/config/autoregister.properties",
			#"/opt/agents/go-agent-2/pipelines",
			#"/var/lib/go-agent-2"
			#]],
	#}
	
	## CRIANDO LINK SIMBOLICO SERVICO "GO AGENT - 2"
	file { '/etc/init.d/go-agent-2':
		ensure => 'link',
		target => '/etc/init.d/go-agent',
		force => true,
	}

	file { '/usr/share/go-agent-2':
		ensure => 'link',
	...
	file{ '/opt/agents/go-agent-2':
		ensure => 'directory',
		owner => 'tfsservice',
		group => 'suporte',
	}

	file{ '/opt/agents/go-agent-2/config':
		ensure => 'directory',
		owner => 'tfsservice',
		group => 'suporte',
	...
	file {"/opt/agents/go-agent-2/config/autoregister.properties":
		ensure   => "file",
		owner    => "tfsservice",
		group    => "suporte",
		...
		require  => [File["/opt/agents/go-agent-2/config"]],
	}
	...
	file { '/var/lib/go-agent-2':
		ensure => 'link',
		target => '/opt/agents/go-agent-2',
		force => true,
		require => [File["/opt/agents/go-agent-2"]],		
	}
	
	file_line {"go_server_url_go_agent_2":
		path => "/etc/default/go-agent-2",
		line => "export GO_SERVER_URL=https://infogocd.ogmaster.local:8154/go",
		match => "^GO_SERVER_URL.*$",
		require => File['/etc/default/go-agent-2'],
	}
	
	file_line {"agent_work_dir_go_agent_2":
		path => "/etc/default/go-agent-2",
		line => "export AGENT_WORK_DIR=/opt/agents/go-agent-2",
		match => "^AGENT_WORK_DIR.*$",
		require => File['/etc/default/go-agent-2'],
	}
	
	file_line {"java_home_go_agent_2":
		path => "/etc/default/go-agent-2",
