class profile::agent_gocd_aws inherits profile::base{		
	...
	include profile::install_docker
	  ...
	  exec {'gocd_agent':
			command  => 'docker run --rm -d -v /var/run/docker.sock:/var/run/docker.sock --add-host=infodevops3:172.17.37.109 --dns=172.17.32.98 --dns-search=ogmaster.local renatoadsumus/gocd_agent:latest',       	
			path => ['/usr/bin',],
